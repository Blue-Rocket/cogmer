...
case "$state" in
  installing)
    echo "claude-team is still arriving — started ${age}s ago (${detail})."
    echo "Try again in a moment; it is usually seconds."
    ;;
  failed)
    echo "claude-team could not be installed, and is not being retried yet."
    echo
    echo "  ${detail}"
    echo
    echo "It will try again about an hour after the failure. To try now, start a new"
    echo "Claude Code session after removing ~/.claude-team/install-state."
    ;;
  stalled)
  ...
  *)
    ...
    echo "new session should be enough. If nothing changes, ~/.claude-team/install.log"
