...
Architectural patterns this project follows. Each is stated so that it applies to any
project, and none names a part of this one, so this list does not go stale when a
detail of the project changes. `CLAUDE.md` imports this file, so every session starts
with it.

P-01. Design for the case you have, and never build a ceiling you cannot remove. Work
...
P-02. Do not abstract for a consumer that does not exist. An interface written for a
...
P-03. Every centralized component traces to a recorded decision, is disclosed to the
user it affects, and benefits that user rather than the maintainer. A compromise of
privacy or decentralization that fails one of the three is a gap to close, not a
reason to forbid the component.

P-04. A change to a stored format migrates existing stores. A statement that creates a
...
P-05. Add complexity only when a test shows the need. A mechanism kept for a problem
nobody has observed costs its full price now for a benefit that may never arrive.

P-06. An integration fails open. When a hook or plugin fails, the host it extends
...
P-07. Frame untrusted input by classification, never by a static delimiter. Text from
...
P-08. Loopback is not a security boundary. A web page open in the same machine's
...
P-09. A secret never shares a file with anything that gets printed or shared. A file
...
P-10. Authentication, admission and verification are three questions, and each is
...
P-11. A signed or immutable record is never rewritten. Its identifier, author and
...
P-12. Persist before publishing, and reserve an identifier before publishing anything
that uses it. The reverse can announce something with no durable record of it, which
a restart then contradicts.

P-13. Key on stable identifiers, never on names people choose. Names collide, get
changed and get guessed, and a record keyed on one attaches to the wrong thing when
that happens.

P-14. Confirm an effect by observing it, never at the moment it is sent. A reply can be
